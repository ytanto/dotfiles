...
- `/review-feedback https://github.com/owner/repo/pull/293` → 指定したPRに対して実施
...
- 引数（`${ARGUMENT}`）が PR URL（`https://github.com/{owner}/{repo}/pull/{number}` 形式）の場合: URL から `{owner, name, prNumber}` を抽出
- 引数が数字のみの場合: PR 番号として扱い、リポジトリは `gh repo view --json owner,name` で取得
- 引数がない場合: `gh pr view --json number,headRepository,headRepositoryOwner` で現在のブランチに紐付く PR とリポジトリ情報を取得

owner / name はハードコードしないこと。

### 2. 未resolveのレビュースレッドを全件取得
...
### 10. PR本体に最終コメント（Claudeレビュー対応時のみ）

step 2 で控えた `author.login` に Claude bot（例: `claude[bot]`）が含まれていた場合のみ実行する。人間のレビュアーのみへの対応の場合はこのステップをスキップする。
