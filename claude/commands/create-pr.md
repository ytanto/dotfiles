...
6. **PR本文の作成**
   ...
   - **シンプルに書くこと**。コミット粒度の機械的な羅列ではなく、レビュアー視点で「何が変わるか」が伝わる粒度にまとめる
   - 変更内容にコードベースの細かな記載は不要
      - APIを追加・変更した場合：対象APIと変更点を明記
      - 機能を追加・変更した場合：追加・変更した機能仕様を明記
   - **書かないこと**
      - リファクタリング観点（共通化、責務分離、可読性向上など内部構造の話）。挙動が変わらない整理は基本的に省く
      - 今回のスコープで新規追加した処理に対する不具合修正。スコープ内で生じて閉じたものは「不具合」ではなく実装の一部なので、独立した fix として書かない
      - 些細な見た目調整など、機能仕様の理解に寄与しない雑多な内容
